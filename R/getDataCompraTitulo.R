#' @title get data
#' @description Funcao para recuperar dados do tesouro direto que estao na pasta data com o nome 'compratesourodireto.csv'
...
getDataCompraTitulo <- function(updateDataSeNaoDisponivel = FALSE) {
  ...
  dataCarregamentoCompraTesouroDireto <<- format(Sys.Date(), "%d/%m/%Y %H:%M")

  tdData <- read.csv2("data/compratesourodireto.csv", sep = ";")
  ...
  tdData$Data.Resgate <- as.Date(tdData$Data.Resgate, format = "%d/%m/%Y")


  #ordenando pela data Base
  tdData <- tdData[order(tdData$Data.Resgate),]
